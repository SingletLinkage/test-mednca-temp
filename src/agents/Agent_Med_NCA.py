...
class Agent_Med_NCA(Agent_Multi_NCA):
    ...
    def get_outputs(self, data, full_img=False, returnInference=False, **kwargs):
        ...
        min_dim = self.exp.get_from_config('min_dimension')
        try:
            min_dim = self.exp.get_from_config('min_dimension')
            if min_dim is None:
                min_dim = 4
        except:
            min_dim = 4  # Use default value if not found in config

    
    # Create down-scaled image, ensuring minimum dimensions
        down_scale_factor = 4  # The factor used for downscaling
        down_scaled_size = (
            max(min_dim, int(inputs.shape[1] / down_scale_factor)),
            max(min_dim, int(inputs.shape[2] / down_scale_factor))
        )

        # Create down-scaled image
        # down_scaled_size = (max(1, int(inputs.shape[1] / 4)), max(1, int(inputs.shape[2] / 4)))
    ...
    def resize4d(self, img, size):
        """
        Resize a 4D tensor batch of images to the given size.
        This is causing the divide by zero error because one of the dimensions in size might be 0.
        
        Args:
            img: Tensor of shape [batch_size, channels, height, width]
            size: Tuple containing the target (height, width)
        
        Returns:
            Resized tensor
        """
        # Make sure size is valid (no zeros)
        if isinstance(size, tuple) and (size[0] <= 0 or size[1] <= 0):
            raise ValueError(f"Invalid resize dimensions: {size}. Both dimensions must be positive.")
        
        # Handle different input formats
        if len(img.shape) == 4:  # [batch, channel, height, width]
            batch_size, channels, _, _ = img.shape
            resized = torch.zeros((batch_size, channels, size[0], size[1]), device=img.device)
            
            # Use standard PyTorch resize instead of torchio
            for b in range(batch_size):
                for c in range(channels):
                    # Use F.interpolate for resizing - much simpler and more reliable
                    single_channel = img[b, c:c+1]  # Keep dim for interpolate
                    resized[b, c] = torch.nn.functional.interpolate(
                        single_channel.unsqueeze(0), 
                        size=size, 
                        mode='bilinear', 
                        align_corners=False
                    ).squeeze(0).squeeze(0)
            ...
            # For other tensor shapes (like 3D tensors)
            return torch.nn.functional.interpolate(
                img.unsqueeze(0) if len(img.shape) == 3 else img,
                size=size,
                mode='bilinear',
                align_corners=False
            ).squeeze(0) if len(img.shape) == 3 else None