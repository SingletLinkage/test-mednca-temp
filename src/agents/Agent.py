...
class BaseAgent():
    ...
    #         return loss_log
    def test(self, loss_f, save_img = None, tag='test/img/', pseudo_ensemble=False, showResults=False, **kwargs):
        """Evaluate model on testdata by merging it into 3d volumes first
            TODO: Clean up code and write nicer. Replace fixed images for saving in tensorboard.
            #Args
                dataset (Dataset)
                loss_f (torch.nn.Module)
        ...
        with torch.no_grad():
            ...
            # Check if we're working with 3D or 2D data
            is_3d_dataset = hasattr(dataset, 'slice')
            ...
            for i, data in enumerate(dataloader):
                ...
                # Run inference 10 times to create a pseudo ensemble
                if pseudo_ensemble: # 5 + 5 times
                    # ... existing pseudo ensemble code ...
                    pass

                # --------------- 2D ---------------------
                if is_3d_dataset and dataset.slice is not None:
                    # ... existing 2D dataset processing code ...
                    pass
                # --------------------------------- 3D ----------------------------
                else: 
                    patient_3d_image = outputs.detach().cpu()
                    ...
                    for m in range(patient_3d_image.shape[-1]):
                        ...
                        if True: 
                            ...
                            # Handle different shapes for 2D vs 3D data
                            if len(patient_3d_real_Img.shape) >= 5:
                                # 3D data with dimension for slices
                                display_real_img = patient_3d_real_Img[:,:,:,5:6,:]
                                display_pred_img = patient_3d_image[:,:,:,5:6,:]
                                display_label_img = patient_3d_label[:,:,:,5:6,:]
                            else:
                                # 2D data without slice dimension
                                display_real_img = patient_3d_real_Img
                                display_pred_img = patient_3d_image
                                display_label_img = patient_3d_label
                                
                            self.exp.write_img(str(tag) + str(patient_id) + "_" + str(len(patient_3d_image)), 
                            convert_image(
                                self.prepare_image_for_display(display_real_img.detach().cpu()).numpy(), 
                                self.prepare_image_for_display(display_pred_img.detach().cpu()).numpy(), 
                                self.prepare_image_for_display(display_label_img.detach().cpu()).numpy(), 
                                encode_image=False), self.exp.currentStep)

            # If 2D and has slices attribute
            if is_3d_dataset and dataset.slice is not None:
                # ... existing 2D dataset finalizing code ...
                pass
